...
def Clustering(lambd_stab, phi_stab,distcl):
    ...
    myClus = fcluster(Z,distcl,criterion='distance')
...
def plotframemodes(mult, MOD, lh, lv, Expfreq):
    ...
    ax1.plot(xde,yun,'-bo',linewidth=3)
    ax1.plot(xun,yun,'--k')
    ax1.set_title('Modo 1 - Freq. : '+str(round(float(Expfreq[0]), 2)))
    ax1.set_ylim([-0.1,1])
    
    ax2 = plt.subplot(1,3,2)
    xde,yde = convert_mode(MOD[:,1], lh, lv)
    ax2.plot(xde,yun,'-bo',linewidth=3)
    ax2.plot(xun,yun,'--k')
    ax2.set_title('Modo 2 - Freq. : '+str(round(float(Expfreq[1]), 2)))
    ax2.set_ylim([-0.1,1])
    ax3 = plt.subplot(1,3,3)

    
    xde,yde = convert_mode(MOD[:,2], lh, lv)
    ax3.plot(xde,yun,'-bo',linewidth=3)
    ax3.plot(xun,yun,'--k')
    ax3.set_title('Modo 3 - Freq. : '+str(round(float(Expfreq[2]), 2)))
    ax3.set_ylim([-0.1,1])
    plt.tight_layout()
    plt.show()
